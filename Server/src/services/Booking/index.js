...
const createBooking = async (userId, data) => {
  ...
  if (!details || details.length === 0) {
    return { message: 'Chưa chọn phòng/dịch vụ' };
  }
  ...
  for (const d of details) {
    if (d?.serviceId) {
      ...
      if (!service) return { message: 'Service không tồn tại' };
      ...
      if (!roomType) return { message: 'RoomType không tồn tại' };
      ...
      if (available < d.quantity) {
        return { message: `Không đủ phòng cho loại ${roomType.name}` };
      }
  ...
  if (providerSet.size > 1) {
    return { message: 'Booking chỉ được từ 1 nhà cung cấp' };
  }
